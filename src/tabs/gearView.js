(function(global) {
    ...
    function buildTabColumnsAndMain(config) {
        ...
        function buildRecipeDropdownHTML() {
            var html = '<div class="recipe-dropdown-wrapper"><div class="recipe-dropdown" id="gear-recipe-dropdown"><button class="recipe-dropdown-trigger" id="gear-recipe-dropdown-trigger"><span id="gear-recipe-dropdown-label">All (Profit Overview)</span><span class="dd-arrow">&#9660;</span></button><div class="recipe-dropdown-menu">';
            ...
            html += '</div></div><button class="recipe-back-btn" id="gear-recipe-back" style="display:none;" title="Back to all recipes">&#8592; All</button></div>';
            return html;
        }
        ...
        function buildIngHTML(ing, recipeId, totalQty, depth) {
            ...
            var html = "";
            if (ing.type === "bop") {
                html += '<div class="sourcing-row fixed"' + indent + "><span>" + totalQty + "x " + ing.item + '</span><span class="cost" style="color:#f59e0b">BOP</span></div>';
                ...
                html += '<div class="sourcing-row"' + indent + '><span class="opt" id="' + baseId + '_buy_label">' + totalQty + "x " + ing.item + ' (buy)</span><span class="opt" id="' + baseId + '_buy_price">-</span></div>';
                html += '<div class="sourcing-row"' + indent + '><span class="opt" id="' + baseId + '_craft_label">' + totalQty + "x " + ing.item + ' (craft)</span><span class="opt" id="' + baseId + '_craft_price">-</span></div>';
                ing.craftFrom.mats.forEach(function(mat) { html += buildIngHTML(mat, recipeId, totalQty * mat.qty, depth + 1); });
            } else {
                html += '<div class="sourcing-row fixed"' + indent + "><span>" + totalQty + "x " + ing.item + '</span><span class="cost" id="' + baseId + '">-</span></div>';
            }
            return html;
        }

        function buildRecipeDetailHTML(recipe) {
            var html = '<div id="gear-view-' + recipe.id + '" class="gear-view" style="display:none"><div class="card"><h3>' + recipe.name + "</h3><div class=\"sourcing\"><div class=\"sourcing-title\">Ingredients</div>";
